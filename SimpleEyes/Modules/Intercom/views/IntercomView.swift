...
/// 语音对讲主视图
///
/// 使用 MVVM 架构模式，通过 IntercomViewModel 管理对讲状态和音频流
/// 功能包括：
/// - 在线设备选择和加载
/// - 对讲状态实时展示（空闲/连接中/已连接/通话中/错误）
/// - 对讲控制（开始/停止）
/// - 麦克风音频采集和播放
/// - 错误处理和重试机制
struct IntercomView: View {
    ...
    var body: some View {
        ...
        .navigationBar(
            ...
            displayMode: .inline,
            trackingParameters: [
                "deviceId": viewModel.selectedDevice?.deviceId ?? "",
                "deviceName": viewModel.selectedDevice?.name ?? "",
                "intercomStatus": viewModel.intercomStatus.displayText,
                "from": "device_detail"
            ]
        )
    }
